...
**Внимание!** В процессе установки будут безвозвратно активированы аппаратные механизмы защиты, что сделает невозможным дальнейшую перепрошивку стандартными способами. Прошивка возможна только через предоставленный инсталлер при наличии лицензии.
...
Для прошивки микроконтроллера ESP32-C3 используется программа, предназначенная для однократной записи защищённой прошивки PS5 Smart Cooler (обновление прошивки поддерживается).
...
4. После обнаружения платы программа считает её уникальный идентификатор (Device ID) и проверит наличие этого ID в удалённой базе данных.
...
**ВАЖНО!** При первой прошивке платы (плата новая, ранее не прошивалась) активация может пройти безуспешно, поскольку плата находится в режиме BOOT. Отключите плату от USB и снова подключите **без удерживания кнопки BOOT**. Активация должна пройти успешно.
...
- **OUTPUT (BOOST FAN SPEED)** - скорость, которую устройство подаёт на вентилятор (отображается синим цветом). **В данной бесплатной версии этот параметр не активен** (функция управления в разработке у автора).
...
- **Кнопки режимов** внизу: MANUAL, OFFSET, TARGET, GRAPH. **В данной версии неактивны** (предназначены для будущей платной прошивки).
...
Плата представлена в виде расширения для ESP32-C3 Mini. Вам необходимо совместить контакты так, чтобы пины питания и сигнальные совпали с назначением в прошивке.
